...
def build_zip(args):
    ...
    # 【核心修改 2】由于前面精简了 ABI_TO_ARCH，这里只会循环并编译 arm64-v8a 架构
    ...
    # 【核心修改 3】使用 os.walk 替换 Path.walk，完美解决低版本 Python 运行报错
    for root, _, fns in os.walk(module_path):
        p = Path(root)
        for fn in fns:
            if fn == 'mazoku':
                continue
    ...
    # 【核心修改 4】压缩 ZIP 阶段：同样使用 os.walk 替换掉 Path.walk
    with zipfile.ZipFile(output_path, 'w', compression=zipfile.ZIP_DEFLATED) as out_zip:
        for root, dns, fns in os.walk(module_path):
            p = Path(root)
            for dn in dns:
                d = p / dn
    ...
    # 【核心修改 5】调试符号打包阶段：同样使用 os.walk 替换掉 Path.walk
    if args.save_debug:
        ...
        with zipfile.ZipFile(symbols_out, 'w', compression=zipfile.ZIP_DEFLATED) as out_zip:
            for root, dns, fns in os.walk(UNSTRIPPED_OUTPUT_DIR):
                p = Path(root)
                for dn in dns:
                    d = p / dn
